...
{
 "id": "evt_pilgrim_encounter", "title": "Yolun Hacısı", "weight": "0.7",
 "cooldown": 8, "kind": "Yol · insan · mizaç",
 "flavor": "Elinde bir asa, sırtında hafif bir denkle yürüyen biri var - doruğa mı, kıyıya mı "
           "gittiğini söylemiyor.",
 "trigger": "Koşulsuz.",
 "options": [
   {"label": "Yanına al, birkaç gün eşlik et",
    "effects": [("PROVISIONS", -2), ("SET_FLAG", 0, "pilgrim_escorted"),
                ("UNLOCK_EVENT", 0, "evt_pilgrim_blessing")]},
   {"label": "Yol tarif et, kendi hâline bırak", "outcomes": [
     {"label": "Tarifin doğru çıkıyor - hacı doğru yöne, memnun ayrılıyor", "weight": "1.0",
      "condition": "Bilgelik check başarılı (kıl payı da sayılır)",
      "effects": [("MORALE", 4), ("SET_FLAG", 0, "pilgrim_escorted"),
                  ("UNLOCK_EVENT", 0, "evt_pilgrim_blessing")]},
     {"label": "Tarifin şüpheli çıkıyor - hacı sessizce ayrılıyor", "weight": "1.0",
      "condition": "Bilgelik check başarısız",
      "effects": [("MORALE", -1)]},
   ]},
   {"label": "Görmezden gel, yoluna devam et",
    "effects": [("MORALE", -1)]},
 ],
 "note": "İki İnanış'ın (bkz. Kilise ekranının iki yaratılış efsanesi) yoldaki karşılığı - "
         "hacı kervana katılmak istemez, yalnızca eşlik ister, o yüzden bu kart TRIGGER_RECRUIT "
         "açmaz. Eşlik etmek erzak yer ama garanti bir teşekkür (bkz. evt_pilgrim_blessing) "
         "açar; yol tarif etmek bedava ama Bilgelik'e bahis - aynı ödülü riske girerek de "
         "alabilirsin. evt_wanderer_revenge'in \"karar gecikmeli sonuç doğurur\" deseninin "
         "olumlu ucu: bu kez fatura değil bir karşılık geliyor.",
},
{
 "id": "evt_pilgrim_blessing", "title": "Hacının Vedası", "weight": "3.0",
 "cooldown": 0, "kind": "ZİNCİR · yalnızca tetiklenir",
 "repeat": "ömürde bir kez",
 "flavor": "Eşlik ettiğin hacı, doruğa ya da kıyıya vardığı günü sana yollamış.",
 "trigger": "Yalnızca <b>pilgrim_escorted</b> bayrağı varsa ve zincir açıldıysa.",
 "options": [
   {"label": "Kutsamayı kabul et",
    "effects": [("STRESS", -8), ("MORALE", 5), ("CLEAR_FLAG", 0, "pilgrim_escorted")]},
 ],
 "note": "Tek seçenekli - bir bedel değil bir mükafat olduğu için üzerinde karar verilecek "
         "bir şey yok. Stresin ücretsiz düştüğü az sayıdaki yerden biri (bkz. Yol Kenarı "
         "Türbesi), ama kamp/ziyafetin aksine bunu tetikleyen erken bir seçim - iyilik yapmak.",
},
# ---------------------------------------------------------------- İÇ DÜNYA
{
