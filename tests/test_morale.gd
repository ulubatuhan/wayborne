...
func run(t) -> void:
	...
	_test_departure_temperament_bonus(t)
...
## Faz 17 PR-3'ün tamamlanan ikinci kancası: "Güven Verici" adının kendisi
## zaten bir moral vaadiydi, huy yalnızca savaşta bir dodge puanıydı - artık
## kervanın çıkış moraline de taşıyor. Parti çapında toplanıyor (yalnızca
## lider değil) ve tavanlı, tek bir huy hardship/stres payını ezmesin diye.
func _test_departure_temperament_bonus(t) -> void:
	var neutral := GameSession.new(500, 60)
	t.eq(neutral.get_departure_temperament_bonus(), 0, "huysuz partide mizaç payı sıfır")

	var reassured := GameSession.new(500, 60)
	reassured.change_stress(GameSession.MAX_STRESS)
	var calming := CharacterData.create("Yatıştırıcı", CultureCatalog.PORT, CharacterStats.new())
	calming.grant_trait(TraitCatalog.REASSURING, 0)
	reassured.party.append(calming)
	var wound_up := GameSession.new(500, 60)
	wound_up.change_stress(GameSession.MAX_STRESS)
	t.ok(
		reassured.get_departure_morale() > wound_up.get_departure_morale(),
		"Güven Verici bir yoldaş çıkış moralini yükseltir"
	)

	var jumpy_session := GameSession.new(500, 60)
	var jumpy := CharacterData.create("Tedirgin", CultureCatalog.PORT, CharacterStats.new())
	jumpy.grant_trait(TraitCatalog.OFF_PUTTING, 0)
	jumpy_session.party.append(jumpy)
	t.ok(
		jumpy_session.get_departure_temperament_bonus() < 0,
		"İtici bir yoldaş mizaç payını negatife çeker"
	)
	t.ge(
		float(jumpy_session.get_departure_temperament_bonus()),
		float(-GameSession.DEPARTURE_TEMPERAMENT_BONUS_CAP),
		"mizaç payı kendi tavanının altına inmez"
	)
