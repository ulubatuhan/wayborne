...
func run(t) -> void:
	...
	_test_city_gold_earn_spend_and_clamp(t)
	_test_city_gold_decays_toward_default(t)
	_test_city_gold_save_round_trip(t)
...
## Faz 17 PR-5: şehrin kendi hazinesi. Satış artık bedava para değil -
## şehir kendi kesesinden öder, alım kesesini doldurur; ikisi de MAX'ta
## ve 0'da kenetlenir.
func _test_city_gold_earn_spend_and_clamp(t) -> void:
	var conditions := MarketConditions.new()
	t.eq(
		conditions.get_city_gold("loc_a"), int(round(MarketConditions.CITY_GOLD_DEFAULT)),
		"dokunulmamış şehir varsayılan hazinede"
	)

	conditions.earn_city_gold("loc_a", 200)
	t.ok(
		conditions.get_city_gold("loc_a") > int(round(MarketConditions.CITY_GOLD_DEFAULT)),
		"alım şehri zenginleştirir"
	)

	conditions.earn_city_gold("loc_a", 100000)
	t.eq(
		conditions.get_city_gold("loc_a"), int(round(MarketConditions.CITY_GOLD_MAX)),
		"hazine MAX'ta kenetlenir"
	)

	conditions.spend_city_gold("loc_a", 100000)
	t.eq(conditions.get_city_gold("loc_a"), 0, "hazine 0'da kenetlenir, eksiye düşmez")

	# Başka şehir etkilenmemeli - tıpkı arz-talep baskısı gibi yerel.
	t.eq(
		conditions.get_city_gold("loc_b"), int(round(MarketConditions.CITY_GOLD_DEFAULT)),
		"başka şehrin hazinesi etkilenmez"
	)

func _test_city_gold_decays_toward_default(t) -> void:
	var drained := MarketConditions.new()
	drained.spend_city_gold("loc_a", int(MarketConditions.CITY_GOLD_DEFAULT))
	t.eq(drained.get_city_gold("loc_a"), 0, "hazine tükendi")

	for day in range(1, 60):
		drained.advance_day(day)
	t.ok(
		drained.get_city_gold("loc_a") > 0, "zamanla hazine kendi tabanına doğru toparlanır"
	)

	var flush := MarketConditions.new()
	flush.earn_city_gold("loc_a", int(MarketConditions.CITY_GOLD_MAX))
	for day in range(1, 200):
		flush.advance_day(day)
	t.ok(
		is_equal_approx(
			float(flush.get_city_gold("loc_a")), MarketConditions.CITY_GOLD_DEFAULT
		),
		"fazla dolan hazine de yeterince beklenirse tabana geri süzülür"
	)

func _test_city_gold_save_round_trip(t) -> void:
	var conditions := MarketConditions.new()
	conditions.earn_city_gold("loc_a", 300)
	conditions.spend_city_gold("loc_b", 150)

	var restored := MarketConditions.new()
	restored.load_from_dict(conditions.to_save_dict())

	t.eq(
		restored.get_city_gold("loc_a"), conditions.get_city_gold("loc_a"),
		"zenginleşmiş hazine kayıttan döner"
	)
	t.eq(
		restored.get_city_gold("loc_b"), conditions.get_city_gold("loc_b"),
		"boşalmış hazine kayıttan döner"
	)

	var legacy := MarketConditions.new()
	legacy.load_from_dict({})
	t.eq(
		legacy.get_city_gold("loc_a"), int(round(MarketConditions.CITY_GOLD_DEFAULT)),
		"eski (city_gold alanı olmayan) kayıt varsayılan hazineyle açılır"
	)
