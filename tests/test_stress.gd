...
func run(t) -> void:
	...
	_test_road_stress_respects_constitution(t)
	_test_make_camp(t)
...
## Faz 17 PR-3'ün tamamlanan ilk kancası: yolun günlük stres payı artık
## kişi başına Demir/Zayıf Bünye'nin stress_resistance_bonus'unu okuyor -
## `change_stress()`'in kendisi (famine/kamp/ziyafet/olay) hâlâ uniform,
## yalnızca yolun kendi günlük payı kişiye göre bükülüyor. Tek gün
## yerine on gün boyunca çekiliyor - gerçek bir seferin ölçeği, tek bir
## günün değil (bkz. CLAUDE.md Stress Rules'un on iki seferlik birikim
## tablosu, aynı disiplin).
func _test_road_stress_respects_constitution(t) -> void:
	var session := GameSession.new(500, 20, 1)
	var sturdy := CharacterData.create("Sağlam", CultureCatalog.NOMAD, CharacterStats.new())
	sturdy.grant_trait(TraitCatalog.IRON_CONSTITUTION, 0)
	var frail := CharacterData.create("Narin", CultureCatalog.NOMAD, CharacterStats.new())
	frail.grant_trait(TraitCatalog.FRAIL_CONSTITUTION, 0)
	session.party = [sturdy, frail]

	for _day in 10:
		session._apply_road_stress_day()
	t.ok(sturdy.stress < frail.stress, "Demir Bünye seferi daha az stresle geçirir")
	t.ok(sturdy.stress > 0, "gün yine de iz bırakır - hiçbir huy günlük yıpranmayı sıfırlamaz")
