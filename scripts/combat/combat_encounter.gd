...
## bulur.
##
## `kind` (Faz 17 PR-7) makine tarafından okunabilir bir sabit - `text`
## zaten `tr()`'den geçmiş, dile göre değişir, o yüzden savaş animasyon
## katmanının "bu bir kritik mi, bir düşüş mü" diye çevrilmiş metni
## karşılaştırması gerekmiyor (`EnemyCatalog.KIND_WILDLIFE` gibi diğer
## "tür" sabitleriyle aynı disiplin).
signal unit_barked(unit, text, kind)  # CombatUnit, String, String

const BARK_HIT: String = "hit"
const BARK_CRIT: String = "crit"
const BARK_REFUSE: String = "refuse"
const BARK_DEATHS_DOOR: String = "deaths_door"
const BARK_SURVIVED: String = "survived"
const BARK_KILLED: String = "killed"
const BARK_DOWNED: String = "downed"
## Faz 17 PR-7 eksiği: motorun bir kaçırma barkı hiç yoktu - isabet/kritik
...
## Simetrik tavan - sağlam huylar da bu ihtimali sıfırlayamaz, tıpkı
## Karizma'nın sıfırlayamaması gibi (bkz. Progression Rules'un ölçümü).
const MAX_STRESS_REFUSAL_CHANCE: int = 95
...
func _try_refuse_order(unit: CombatUnit) -> bool:
	if not unit.is_stressed:
		return false
	var chance := clampi(
		STRESS_REFUSAL_CHANCE - unit.composure + unit.refusal_bonus,
		MIN_STRESS_REFUSAL_CHANCE, MAX_STRESS_REFUSAL_CHANCE
	)
	if _rng.randi_range(1, 100) > chance:
		return false
	_emit_log(tr("CBT_LOG_REFUSE") % unit.display_name)
	unit_barked.emit(unit, tr("CBT_BARK_REFUSE"), BARK_REFUSE)
	return true
...
func _resolve_on_target(unit: CombatUnit, skill: CombatSkill, target: CombatUnit) -> void:
	...
	if _rng.randi_range(1, 100) > hit_chance:
		...
		unit_barked.emit(target, tr("CBT_BARK_MISS"), BARK_MISS)
		return
	...
	if is_crit:
		...
		unit_barked.emit(target, tr("CBT_BARK_CRIT"), BARK_CRIT)
	else:
		_emit_log(tr("CBT_LOG_HIT") % [
			unit.display_name, skill.display_name, target.display_name, taken
		])
		unit_barked.emit(target, tr("CBT_BARK_HIT"), BARK_HIT)
...
func _report_damage_outcome(target: CombatUnit, outcome: String) -> void:
	match outcome:
		"deaths_door":
			...
			unit_barked.emit(target, tr("CBT_BARK_DEATHS_DOOR"), BARK_DEATHS_DOOR)
		"survived_deathblow":
			_emit_log(tr("CBT_LOG_DEATHBLOW_SURVIVED") % target.display_name)
			unit_barked.emit(target, tr("CBT_BARK_SURVIVED"), BARK_SURVIVED)
		"killed":
			_emit_log(tr("CBT_LOG_KILLED") % target.display_name)
			unit_barked.emit(target, tr("CBT_BARK_KILLED"), BARK_KILLED)
		...
		"downed":
			...
			unit_barked.emit(target, tr("CBT_BARK_DOWNED"), BARK_DOWNED)
			_repack(_side_of(target))
