...
var _cargo_panel: CaravanCargoPanel = null
...
func _build_overview_button() -> void:
	_overview_wagon_button = Button.new()
	_overview_wagon_button.text = tr("UI_CARAVAN_OVERVIEW_BUTTON")
	_overview_wagon_button.pressed.connect(_on_overview_pressed)
	var container := _sell_wagon_button.get_parent()
	container.add_child(_overview_wagon_button)
	container.move_child(_overview_wagon_button, _sell_wagon_button.get_index() + 1)
...
## Kervan Envanteri Rules'un #22 notunun kalan parçası: hangi vagonun ne
## taşıdığını bilerek dağıtmak için ayrı bir ekran (bkz. CaravanCargoPanel) -
## dökümün hemen yanına, aynı sıra-düğmesi deseniyle.
func _build_cargo_button() -> void:
	var button := Button.new()
	button.text = tr("UI_CARGO_PANEL_BUTTON")
	button.pressed.connect(_on_cargo_pressed)
	var container := _overview_wagon_button.get_parent()
	container.add_child(button)
	container.move_child(button, _overview_wagon_button.get_index() + 1)

func _on_cargo_pressed() -> void:
	if _cargo_panel != null:
		return
	_cargo_panel = CaravanCargoPanel.new()
	add_child(_cargo_panel)
	_cargo_panel.closed.connect(_on_cargo_closed)
	_cargo_panel.setup(_session)
