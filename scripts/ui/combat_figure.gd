...
## şey oldu" anını taşıyan geçici bir görsel tepki. `CombatUnitSlot`/
...
## CombatPanel._apply_pending_animation), tıpkı `show_bark()` gibi.
func set_flash(color: Color) -> void:
	modulate = color

## Saldıran figür hedefine doğru kısa bir hamle yapar - `_draw()`'ın en
