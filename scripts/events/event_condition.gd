...
enum Op {
	...
	HAS_FLAG,
	NOT_HAS_FLAG,
	## Faz 17 PR-6 eksiği: haydut fidyesini danger'a göre kademelemek için
	## eklendi. GREATER_EQUAL + LESS_EQUAL ile aynı eşiği paylaşan iki bandın
	## sınırda ikisinin de "available" sayılması (bkz. EventEngine.
	## resolve_outcome'ın ağırlıklı çekimi - iki sonuç birden uygunsa sonuç
	## artık belirlenimci değil, rastgele oluyor) tam bu yüzden istenmiyor.
	## LESS_THAN'in kesin, örtüşmeyen bir üst sınırı var - aynı eşiğe
	## GREATER_EQUAL'lı bir alt banda asla çakışmıyor.
	LESS_THAN,
}
